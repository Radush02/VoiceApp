...
public class ChannelService implements ChannelServiceImpl {
  ...
  @Override
  public CompletableFuture<Map<String,String>> createChannel(ChannelDTO channel) {
    User u = userRepository.findByUsername(authService.extractUsername()).orElseThrow(() -> new NonExistentException("User not found"));
    ...
    u.getChannels().add(new ChannelMembership(newChannel.getVanityId(), Role.ADMIN));
  ...
  public CompletableFuture<Map<String, String>> joinChannel(InviteDTO inviteDTO) {
    ...
    Invite invite = inviteRepository.findById(inviteDTO.getInviteId())
    ...
    if (invite.getMaxUses() != null && invite.getUses() != null &&
            invite.getUses() >= invite.getMaxUses()) {
      throw new NotPermittedException("This invite has already been used the maximum number of times.");
    }

    User user = userRepository.findByUsername(currentUser)
            .orElseThrow(() -> new NonExistentException("User not found"));
    Channel channel = channelRepository.findByVanityId(invite.getVanityId())
            .orElseThrow(() -> new NonExistentException("Channel not found"));

    boolean alreadyInChannel = user.getChannels().stream()
            .anyMatch(cm -> cm.getVanityId().equals(channel.getVanityId()));
    if (alreadyInChannel) {
      return CompletableFuture.completedFuture(Map.of("Server", channel.getVanityId()));
    }

    user.getChannels().add(new ChannelMembership(channel.getVanityId(), Role.USER));
  ...
  public CompletableFuture<Map<String,String>> createInvite(CreateInviteDTO createInviteDTO) {
    User u = userRepository.findByUsername(authService.extractUsername()).orElseThrow(()->new NonExistentException("User not found"));
    boolean isAdmin = u.getChannels().stream()
            .anyMatch(cm -> cm.getVanityId().equals(createInviteDTO.getVanityId())
